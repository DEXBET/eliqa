...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
        ...
        CTxOut vout = CTxOut(349999.99 * COIN, obfuScationPool.collateralPubKey);
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 Eliqa tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 ELIQA tx -> 1 confirmation
